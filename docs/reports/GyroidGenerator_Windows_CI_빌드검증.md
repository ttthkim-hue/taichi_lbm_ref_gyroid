...
## 6. STEP(OCP) 포함 빌드 (2026-03-20 이후)

- **원인:** 이전 exe는 PyInstaller에 **cadquery/OCP 미포함** → `convert_to_step` 에서 `ImportError` 로 STEP 생략.
- **조치:** CI·`build_windows.bat` 에 **`pip install cadquery`** 고정, `GyroidGenerator.spec` 에 `collect_all("OCP")` + Windows DLL용 `pyi_rth_gyroid_ocp.py` 추가.
- **검증 Run:** https://github.com/ttthkim-hue/taichi_lbm_ref_gyroid/actions/runs/23325807745 (성공, 약 3m20s)
- **exe 크기:** OCP 포함 후 약 **174 MB** (이전 STL-only 약 67 MB).
